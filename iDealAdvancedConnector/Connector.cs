    ...
    public class Connector
    {
        ...
        private class MerchantConfig : ICloneable
        {
            ...
            public object Clone()
            {
                ...
                merchantConfig.MerchantId = this.MerchantId;
                merchantConfig.SubId = this.SubId;
                merchantConfig.merchantReturnUrl = this.merchantReturnUrl;

                merchantConfig.ClientCertificate = this.ClientCertificate;
                merchantConfig.aquirerCertificate = this.aquirerCertificate;

                merchantConfig.acquirerURL = this.acquirerURL;
                merchantConfig.acquirerUrlDIR = this.acquirerUrlDIR;
                merchantConfig.acquirerUrlTRA = this.acquirerUrlTRA;
                merchantConfig.acquirerUrlSTA = this.acquirerUrlSTA;
                merchantConfig.acquirerTimeout = this.acquirerTimeout;

                merchantConfig.ExpirationPeriod = this.ExpirationPeriod;

                merchantConfig.currency = this.currency;
                merchantConfig.language = this.language;
        ...
        {          
                if (Connector.defaultMerchantConfig == null)
                {
                    ...
                    newMerchant.MerchantId = GetAppSetting("MerchantID");
                    newMerchant.SubId = GetAppSetting("SubID");

                    string merchantReturnUrl = GetAppSetting("MerchantReturnURL");
                    if (!Uri.TryCreate(merchantReturnUrl, UriKind.Absolute, out newMerchant.merchantReturnUrl)) throw new UriFormatException("MerchantReturnURL is not in correct format.");
                    ...
                    var acquirerUrl = ConfigurationManager.AppSettings["AcquirerURL"];
                    ...
                    if (!String.IsNullOrEmpty(acquirerUrl))
                    {
                        ...
                        if (!Uri.TryCreate(acquirerUrl, UriKind.Absolute, out newMerchant.acquirerURL)) throw new UriFormatException("AcquirerURL is not in correct format.");
                    ...
                    if (!Int32.TryParse(acquirerTimeout, out newMerchant.acquirerTimeout)) throw new InvalidCastException("AcquirerTimeout is not in correct format.");
                    ...
                    XmlSignature.XmlSignature.RegisterSignatureAlgorithm();
        ...
        public Connector(X509Certificate2 acquirerCertificate = null, X509Certificate2 clientCertificate = null)
        {
            ...
            merchantConfig = (MerchantConfig)DefaultMerchantConfig(acquirerCertificate, clientCertificate).Clone();
