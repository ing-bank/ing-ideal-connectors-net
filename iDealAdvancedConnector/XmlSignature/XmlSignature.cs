...
namespace ING.iDealAdvanced.XmlSignature
{
    ...
    public class XmlSignature
    {
        ...
        /// </summary>
        public static void RegisterSignatureAlgorithm()
        {
            /*
            //unfortunate fix for 3.5
            var type = typeof(CryptoConfig);
            var method = type.GetMethod("AddAlgorithm");
            
            if (method == null)
            {
                var target = type.GetField("defaultNameHT", BindingFlags.NonPublic | BindingFlags.Static);
                var target2 = type.GetField("defaultOidHT", BindingFlags.NonPublic | BindingFlags.Static);
                if (target == null)
                    throw new Exception("No defaultNameHT!");

                Hashtable source = (Hashtable)type.GetProperty("DefaultNameHT", BindingFlags.NonPublic | BindingFlags.Static).GetValue(null, null);
                Hashtable source2 = (Hashtable)type.GetProperty("DefaultOidHT", BindingFlags.NonPublic | BindingFlags.Static).GetValue(null, null);

                if (!source.ContainsKey(XmlDigSignRSASHA256Namespace))
                {
                    source.Add(XmlDigSignRSASHA256Namespace, typeof(RSAPKCS1SHA256SignatureDescription).AssemblyQualifiedName);
                    target.SetValue(null, source);
                }

                //var xmldsig = CryptoConfig.CreateFromName(XmlDigSignRSASHA256Namespace);
                //throw new Exception(xmldsig.GetType().Name);
            }
            else
                method.Invoke(null, new object[] { typeof(RSAPKCS1SHA256SignatureDescription), new string[] { XmlDigSignRSASHA256Namespace } });
             */
        }
